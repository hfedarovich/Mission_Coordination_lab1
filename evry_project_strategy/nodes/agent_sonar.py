...
def run_demo():
    ...
    kw = 0.75
    kv = 1
    flag1 = np.reshape(np.array([4,-4]),(-1,1))
    flag2 = np.reshape(np.array([25,0]),(-1,1)) 
    flag3 = np.reshape(np.array([20,-15]),(-1,1))
    robot_id = int(robot_name[-1])
    ...
    while not rospy.is_shutdown():
        ...
        robot_location = np.reshape(np.array([robot.x,robot.y]),(-1,1))
        ...
        dobs = robot.get_sonar()
        if dobs >= 5:
            Frep = np.zeros((2,1))
        else:
            Frep = np.reshape(-krep/(dobs*dobs)*np.array([np.cos(robot.yaw),np.sin(robot.yaw)]),(-1,1))

        Fatt = katt*(flag - robot_location)
        F = Fatt + Frep
