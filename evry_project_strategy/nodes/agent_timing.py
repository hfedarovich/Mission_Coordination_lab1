...
class Robot:
    ...
    def set_speed_angle(self, linear, angular):
        ...
        """
        cmd_vel = Twist()
        cmd_vel.linear.x = self.constraint(linear, min=-10, max=10)
...
def run_demo():
    ...
    while not rospy.is_shutdown():
        ...
        error = kv*(flag - robot_location)
        angl_error = -robot.yaw + np.arctan2(error[1],error[0])

        velocity = np.linalg.norm(error,2)*np.cos(angl_error)  
        angle = kw*(angl_error) 
